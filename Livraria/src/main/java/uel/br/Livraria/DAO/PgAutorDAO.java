...
public class PgAutorDAO implements AutorDAO{
   ...
   private static final String GET_BY_PNOME_QUERY =
                                "SELECT Pnome, Snome, Nacionalidade FROM livraria.autor " +
                                "WHERE Pnome = ?;";

   private static final String GET_BY_SNOME_QUERY =
                                "SELECT Pnome, Snome, Nacionalidade FROM livraria.autor " +
                                "WHERE Snome = ?;";

   private static final String GET_BY_NACIONALIDADE_QUERY =
                                "SELECT Pnome, Snome, Nacionalidade FROM livraria.autor " +
                                "WHERE Nacionalidade = ?;";
   ...
   public Autor getByPnome(String Pnome) throws SQLException {
      ...
      try (PreparedStatement statement = connection.prepareStatement(GET_BY_PNOME_QUERY)) {
         statement.setString(1, Pnome);
   ...
   public Autor getBySnome(String Snome) throws SQLException {
      ...
      try (PreparedStatement statement = connection.prepareStatement(GET_BY_SNOME_QUERY)) {
         statement.setString(1, Snome);
         try (ResultSet result = statement.executeQuery()) {
               if (result.next()) {
                  autor.setID(result.getInt("ID"));
                  autor.setPnome(result.getString("Pnome"));
   ...
   public Autor getByNacionalidade(String Nacionalidade) throws SQLException {
      ...
      try (PreparedStatement statement = connection.prepareStatement(GET_BY_NACIONALIDADE_QUERY)) {
